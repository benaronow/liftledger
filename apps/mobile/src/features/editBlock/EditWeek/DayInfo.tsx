...
export const DayInfo = ({ day, dIdx, hasErrors, onRequestDelete }: Props) => {
  ...
  const handleMoveDay = (movedDay: Day, dayIdx: number, type: "up" | "down") => {
  ...
  const exerciseCount = day.exercises.reduce(
    (acc, ex) => acc + (ex.addedOn ? 0 : ex.sets.filter((s) => !s.addedOn).length),
    0,
  );
  ...
  return (
    ...
    >
      <Text style={styles.bold}>{`Name: ${day.name} [${exerciseCount}]`}</Text>
      {hasListedExercises &&
        day.exercises
          .filter((ex) => !ex.addedOn)
          .map((ex, i) => (
            <Text style={styles.line} key={ex._id ?? i}>
              {`${i + 1}. ${ex.name} [${ex.sets.filter((s) => !s.addedOn).length}]`}
            </Text>
          ))}
      {hasErrors && (
        <Text style={styles.error}>Must add at least one exercise.</Text>
      )}
    </Info>
  );
};
...
const styles = StyleSheet.create({
  bold: { color: "white", fontSize: FONT.sm, fontWeight: "700" },
  line: { color: "white", fontSize: FONT.sm },
  error: { color: COLORS.danger, fontSize: FONT.sm, fontWeight: "700" },
});
