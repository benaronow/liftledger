...
// `Set`/`CompletedExercise` are type-only: the inline `type` keeps Babel from
// emitting a runtime import named `Set` (shared has no such value export), which
// would otherwise shadow the global `Set` used in `new Set(...)` below.
import { COLORS, type CompletedExercise, type Set } from "@liftledger/shared";
  ...
  return (
    <View style={styles.container}>
      <View style={styles.legend}>
        {gyms.map((gym, i) => (
          <View key={gym} style={styles.legendItem}>
            <View
              style={[
                styles.legendDot,
            ...
            />
            <Text style={styles.legendLabel}>{gym}</Text>
          </View>
        ))}
      </View>
...
const styles = StyleSheet.create({
  ...
  legendLabel: { color: "white", fontSize: FONT.base, fontWeight: "700" },
  chartArea: { flex: 1, paddingLeft: LEFT_PAD },
  axisText: { color: "white", fontSize: 11 },
});
